...
describe("alpha2ToNumeric", () => {
  ...
  describe("enableErrors is true", () => {
    ...
    test("throws an error for a single character input", () => {
      expect(() => alpha2ToNumeric("A", converterOptions)).toThrowError(new Error("Country code does not match expected length"));
    });

    test("throws an error for an input that is too long", () => {
      expect(() => alpha2ToNumeric("ABC", converterOptions)).toThrowError(new Error("Country code does not match expected length"));
    });
  });
});
